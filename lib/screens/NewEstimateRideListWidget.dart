...
class NewEstimateRideListWidget extends StatefulWidget {
  ...
  NewEstimateRideListWidget({required this.sourceLatLog, required this.destinationLatLog, required this.sourceTitle, required this.destinationTitle, this.isCurrentRequest = false, this.servicesId, this.id});
...
class NewEstimateRideListWidgetState extends State<NewEstimateRideListWidget> with WidgetsBindingObserver {
  ...
  final List<String> types = ['Instant', 'Scheduled', 'Hourly', 'Half Day', 'Daily', 'Rental'];

  int selectedPIndex = 0;
  int selectedHIndex = 0;
  DateTime selectedDate = DateTime.now();
  TimeOfDay selectedTime = TimeOfDay.now();

  DateTime selectedDropDate = DateTime.now();
  TimeOfDay selectedDropTime = TimeOfDay.now();

  bool isRound = false;
  payWithSSL(double amount, context) {
    showInformativeDialog(context, amount);
  }

  void showInformativeDialog(BuildContext context, double amount) {
    String selectedType = types[selectedPIndex];
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                'You are about to book a "$selectedType". Please note:',
              ...
              Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'An advance payment of 25% of the full amount is required to confirm your booking.',
                    ),
                  ),
                ],
              ),
              ...
              Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'The remaining 75% payment can be made after the service is completed.',
                    ),
                  ),
                ],
              ),
              ...
              Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Failure to make the advance payment will result in cancellation of the booking.',
                    ),
                  ),
                ],
              ),
              ...
              onPressed: () {
                ...
                Sslcommerz sslcommerz = Sslcommerz(
                    initializer: SSLCommerzInitialization(
                        ...
                        store_id: "abcui675b17e3d0fd4",
                        store_passwd: "abcui675b17e3d0fd4@ssl",
                        total_amount: amount * .25 * calculateMultiplication(),
                        tran_id: "fghdtjhfgjfhjhfjhfgjhg"));

                sslcommerz.addShipmentInfoInitializer(
                    sslcShipmentInfoInitializer: SSLCShipmentInfoInitializer(
                        shipmentMethod: "yes", numOfItems: 5, shipmentDetails: ShipmentDetails(shipAddress1: "Ship address 1", shipCity: "Faridpur", shipCountry: "Bangladesh", shipName: "Ship name 1", shipPostCode: "7860")));
                sslcommerz.addCustomerInfoInitializer(
                    customerInfoInitializer: SSLCCustomerInfoInitializer(
                        ...
                        customerPhone: "0139837574564"));

                sslcommerz.addProductInitializer(
                    sslcProductInitializer:
                        SSLCProductInitializer.WithNonPhysicalGoodsProfile(productName: "productName", productCategory: "productCategory", nonPhysicalGoods: NonPhysicalGoods(productProfile: "Product profile", nonPhysicalGoods: "non physical good")));

                sslcommerz.payNow().then(
                  (value) {
                    print(value.toJson());
                    if (value.status == "VALID") {
                      saveBookingData("${value.amount}");
                    }
                  },
                );
              },
  ...
  Future<void> _selectTime() async {
    ...
    // If the selected date is today, check that the time is not in the past
    ...
    if (selectedDate.day == DateTime.now().day && (picked.hour < now.hour || (picked.hour == now.hour && picked.minute < now.minute))) {
      // Show a snackbar if the selected time is in the past
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please select a time that is in the future')),
      );
    } else {
      setState(() {
        selectedTime = picked;
  ...
  Future<void> _selectDropDate() async {
    final DateTime now = DateTime.now().add(Duration(minutes: 40));
    final DateTime picked = await showDatePicker(
          context: context,
          initialDate: selectedDropDate,
          firstDate: now,
          // Prevent selecting a date before now
          lastDate: DateTime(now.year + 5),
        ) ??
  ...
  Future<void> _selectDropTime() async {
    ...
    final TimeOfDay picked = await showTimePicker(
          context: context,
          initialTime: selectedDropTime,
        ) ??
        selectedDropTime;

    if (selectedDropDate.day == selectedDate.day && (picked.hour < now.hour || (picked.hour == now.hour && picked.minute < now.minute))) {
      // Show a snackbar if the selected time is in the past
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please select a time that is in the future')),
      );
      ...
      children: [
        ...
        Center(
          child: Padding(
            ...
            child: SizedBox(
              height: 50, // Height of the scrollable container
              child: ListView.builder(
                scrollDirection: Axis.horizontal,
                itemCount: types.length,
                itemBuilder: (context, index) {
                  bool isSelected = index == selectedPIndex;
                  ...
                  itemBuilder: (context, index) {
                    return GestureDetector(
                      ...
                      child: Container(
                          ...
                          width: 60,
                          decoration: BoxDecoration(color: selectedHIndex == index ? Colors.red : Colors.grey, borderRadius: BorderRadius.circular(6)),
                          child: Center(
                              child: Text(
                            "${index + 1}h ",
                            style: TextStyle(color: selectedHIndex == index ? Colors.white : Colors.black),
                          ))),
                    );
                  },
                  shrinkWrap: true,
                        ...
                        Row(
                          ...
                          children: [
                            Column(
                              ...
                              children: [
                                Text(
                                  printAmount((e.totalAmount!.toDouble() * calculateMultiplication()).toStringAsFixed(digitAfterDecimal)),
                          ...
                          child: Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                Text(
                                  "${selectedDropDate.day}/${selectedDropDate.month}/${selectedDropDate.year}",
                                  style: TextStyle(fontSize: 16),
                                ),
                              ],
                            ),
                          ...
                          child: Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                Text(
                                  selectedDropTime.format(context),
                                  style: TextStyle(fontSize: 16),
                                ),
                              ],
                            ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            selectedPIndex == 0
                                ...
                                : Text(
                                    "25% Advance Online Payment Required",
        ...
        Padding(
          ...
          child: AppButtonWidget(
            onTap: selectedPIndex != 0 || selectedPIndex != 5
                ? () {
                    payWithSSL(double.parse(mSelectServiceAmount!), context);
                  }
                : () {
                    if (mSelectServiceAmount != null && paymentMethodType != CASH_WALLET && paymentMethodType == WALLET && double.parse(mSelectServiceAmount!) >= mTotalAmount.toDouble()) {
            ...
            text: selectedPIndex == 1 ? "Schedule Ride" : language.bookNow,
            textStyle: boldTextStyle(color: Colors.white),
            width: MediaQuery.of(context).size.width,
          ),
        ),
  ...
  Future<void> saveBookingData(String advance) async {
    ...
    Map req = {
      ...
      "package": types[selectedPIndex].toLowerCase(),
  ...
  calculateMultiplication() {
    return ((selectedPIndex == 1 && isRound)
        ? 2
        : selectedPIndex == 2
            ? selectedHIndex + 1
            : selectedPIndex == 3
